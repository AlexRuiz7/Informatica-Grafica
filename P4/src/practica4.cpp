...
Esfera s(40, 3);
ObjectRev esfera("ply_rev/esfera.ply", 40);
ObjectRev peon("ply_rev/peon.ply");
Tablero t;
vector<Object3d> _objects = {esfera,peon,t};
...
void moveSpotLight(){
	GLfloat position[] = { 10.0f+msl, 0.0, 0, 1.0 };
	GLfloat position_v[] = {0.0, 10.0f+msl, 0.0, 1.0 };
	if(vertical){
		glPushMatrix();
		if(rsl){
...
void moveDirectionalLight(){
	GLfloat position[] = { 5.0f+mdl, 1.0, 0, 1.0 };
	GLfloat position_v[] = {1.0, 5.0f+mdl, 0.0, 1.0 };
	if(vertical_dir){
		glPushMatrix();
		if(rdl){
...
int main(int argc, char **argv){
	...
	// al bucle de eventos)
	glutCreateWindow("Practica 4: Iluminacion y Texturas");
